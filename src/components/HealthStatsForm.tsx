...
interface HealthStats {
    ...
    weight: string; // Keep as string for form input
    height: string; // Keep as string for form input
    gender: string;
    medicalConditions: string;
}

// onNext now takes HealthStats (which might be null if skipped)
const HealthStatsForm = ({ onNext }: { onNext: (stats: HealthStats | null) => void }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        onNext(stats); // Pass the collected stats to the parent (App.tsx)
    };

    const handleSkip = () => {
        onNext(null); // Pass null to indicate skipping
    };
