...
// --- ALL TENSORFLOW/POSEDETECTION IMPORTS ---
import * as poseDetection from "@tensorflow-models/pose-detection";
import "@tensorflow/tfjs-backend-webgl";
import * as tf from "@tensorflow/tfjs";
import styled from "styled-components"; // Assuming you use styled-components here

// --- (All type definitions from your AICoachDemo code) ---
type squatAnalysis = {
...
// --- (All styled-components from your AICoachDemo code, plus the new Footer) ---
...
const PageLayout = styled.div`
    ...
    background-color:rgb(236,252,244); /* Light blue background */
    min-height: 100vh;
...
const ViewOption = styled.button<{ active: boolean }>`
    ...
    &:hover {
        background: ${({ active }) => (active ? "#023658" : "#e0e0e0")};
    }
`;

// NEW: Styled component for the footer (now part of AICoachDemo.tsx)
const Footer = styled.footer`
    width: 100%;
    padding: 5px;
    background-color: #023658; /* Dark blue, matching your PageLayout */
    color: white;
    text-align: center;
    position: sticky; /* Keeps it in flow but "sticks" when scrolled to bottom */
    bottom: 0;
    left: 0;
...
// Define props for AICoachDemo, including the onGoBack function for navigation
interface AICoachDemoProps {
    onGoBack: () => void; // This function will be provided by App.tsx for navigation
}

const AICoachDemo = ({ onGoBack }: AICoachDemoProps) => { // Removed 'AICoachDemoComponent'
    ...
    useEffect(() => {
        ...
        return () => {
            // Cleanup: stop webcam stream and dispose TF model
            if (mediaStream) mediaStream.getTracks().forEach(track => track.stop());
            if (detectorRef.current) detectorRef.current.dispose();
            if (detectionIntervalIdRef.current) clearInterval(detectionIntervalIdRef.current);
        };
    }, [mediaStream]); // Added mediaStream to dependency array as it's used in cleanup

    useEffect(() => {
        if (!mediaStream) return;
        if (detectionIntervalIdRef.current) clearInterval(detectionIntervalIdRef.current);
        detectionIntervalIdRef.current = setInterval(performDetectionAndUpdateUI, DETECTION_AND_UI_UPDATE_INTERVAL_MS);
    }, [mediaStream, performDetectionAndUpdateUI]);

    return (
        <> {/* Use a fragment to wrap both layout and footer */}
            <PageLayout>
                <CameraWrapper>
                    <WebcamVideo
                        ref={videoRef}
                    ...
                    <DemoVideo
                        key={selectedExercise} // Key changes to force remount when exercise changes
                        autoPlay
                        loop
                        muted
                        playsInline
                ...
                <ControlPanel>
                    ...
                    <Label>
                        ...
                        <Select
                            ...
                            onChange={(e) => {
                                ...
                                // Clear buffers when exercise changes
                    ...
                    {/* Conditional rendering for metrics based on selectedExercise */}
            ...
            {/* FOOTER for AICoachDemo with Back button */}
            <Footer>
                <Button
                    onClick={onGoBack} // This calls the function passed from App.tsx
                    className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-8 py-4 rounded-full text-lg shadow-lg transition-all duration-300 hover:scale-105"
                >
